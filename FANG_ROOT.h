// @(#)root/fang:$Id$
// Author: Arik Kreisel (original), refactored for ROOT coding conventions
...
namespace FANG {
   ...
   const Double_t kTwoPi   = 2.0 * kPi;
   const Double_t kFourPi  = 4.0 * kPi;

   /// Physics constants
   const Double_t kDipoleMassSq         = 0.71;    ///< GeV^2, for form factor
   const Double_t kProtonMagneticMoment = 2.793;   ///< mu_p in nuclear magnetons

   /// Numerical tolerances
   const Double_t kPositionTolerance = 1E-6;
   const Double_t kMomentumTolerance = 1E-12;
...
/**
 * \brief Enumeration for detector shape/generation modes
 *
...
*/
   ...
   const Double_t kModePoint  = 2.0;
   const Double_t kModeCircle = 0.0;
...
////////////////////////////////////////////////////////////////////////////////
// Node Structure for Solution Tree
////////////////////////////////////////////////////////////////////////////////
 ...
 */
inline void DeleteTree(Node_t* node)
{
 ...
 */
inline Node_t* CreateFirst(Node_t* node,
 ...
 */
inline Node_t* CreateRight(Node_t* node, Node_t* tmp,
                           const ROOT::Math::PxPyPzMVector& p1,
                           const ROOT::Math::PxPyPzMVector& p2,
                           Double_t weight)
{
 ...
 * \return Pointer to node
 */
inline Node_t* CreateLeft(Node_t* node, Node_t* tmp,
                          const ROOT::Math::PxPyPzMVector& p1,
                          const ROOT::Math::PxPyPzMVector& p2,
                          Double_t weight)
{
 ...
 */
inline void CollectPaths(Int_t nBody, Node_t* node,
 ...
 * \param[out] paths Output: all complete paths
 */
inline void CollectPathsWeights(Int_t nBody, Node_t* node,
 ...
 */
inline Double_t CalcKMFactor(Double_t x, Double_t y)
{
   Double_t arg = (1.0 - x - y) * (1.0 - x - y) - 4.0 * x * y;
   if (arg < 0) {
      ::Warning("CalcKMFactor", "Received negative sqrt argument: %g", arg);
      return 0.0;
   }
   return std::sqrt(arg);
}
 ...
 */
inline void TwoBody(const ROOT::Math::PxPyPzMVector& S,
                    ...
                    ROOT::Math::PxPyPzMVector& p2)
{
   ...
   if ((E1 * E1 - m1 * m1) < 0) {
      ::Error("TwoBody", "E1^2 - m1^2 < 0, E1=%g, m1=%g", E1, m1);
      return;
   }
 ...
 */
inline Bool_t TGenPointSpace(const ROOT::Math::PxPyPzMVector& S1,
                             Double_t m1, Double_t m2,
                             ...
                             ROOT::Math::PxPyPzMVector* pD2)
{
   ...
   if ((CME3 * CME3 - m1 * m1) < 0) {
      ::Error("TGenPointSpace", "CMp3 negative sqrt");
      ::Error("TGenPointSpace", "S1.M()=%g S1.P()=%g", (Double_t)S1.M(), (Double_t)S1.P());
      ::Error("TGenPointSpace", "m1=%g m2=%g", m1, m2);
      solutions = 0;
      return kFALSE;
   }
   ...
   for (Int_t l = 0; l < solutions; l++) {
      ...
      if (std::abs(pD2[l].M() - m2) > kMomentumTolerance) {
         ::Warning("TGenPointSpace", "Mass mismatch: %g != %g",
                   pD2[l].M(), m2);
      }
 ...
 */
inline void TGenVec(Double_t Omega, Double_t Ratio,
                    ...
                    ROOT::Math::XYZVector& vPoint)
{
   ...
   if (Omega > kFourPi || Omega < 0) {
      Omega = kFourPi;
      ::Warning("TGenVec", "Omega out of range, set to 4pi");
   }

   // Validate Ratio for strip mode
   if (Ratio > 1.0) {
      Ratio = 0.0;
      ::Warning("TGenVec", "Ratio out of range, set to 0");
   }
   ...
   if (IsCircle(Ratio)) {
      ...
      if (cst0 > (1.0 - Dcos / 2.0)) {
         cst0 = 1.0 - Dcos / 2.0;
         ::Warning("TGenVec", "Center moved to agree with Omega (near +1)");
      }
      if (cst0 < (-1.0 + Dcos / 2.0)) {
         cst0 = -1.0 + Dcos / 2.0;
         ::Warning("TGenVec", "Center moved to agree with Omega (near -1)");
 ...
 */
inline Int_t GenFANG(Int_t nBody,
                     ...
                     std::vector<Double_t>& vecWi)
{
   ...
   if (mall >= mS) {
      ::Error("GenFANG", "Sum of decay masses (%g) >= parent mass (%g)",
              mall, mS);
      return 0;
   }
